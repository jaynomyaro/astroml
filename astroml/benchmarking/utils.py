...
def measure_gpu_memory() -> float | None:
...
class MemoryMonitor:
    ...
    def __exit__(self, exc_type, _exc_val, _exc_tb):
        ...
        print(
            f"  RSS: {format_memory(self.start_memory['rss_mb'])} -> {format_memory(self.end_memory['rss_mb'])} ({format_memory(abs(rss_delta))} {'increase' if rss_delta > 0 else 'decrease'})"
        )

        if self.gpu_start is not None and self.gpu_end is not None:
            gpu_delta = self.gpu_end - self.gpu_start
            print(
                f"  GPU: {format_memory(self.gpu_start)} -> {format_memory(self.gpu_end)} ({format_memory(abs(gpu_delta))} {'increase' if gpu_delta > 0 else 'decrease'})"
            )
