...
from astroml.features.asset_typing import classify_asset
...
class TransactionGraph:
        ...
        for src in edge_dict:
            ...
            for dst in edge_dict[src]:
                ...
                for txn in edge_dict[src][dst]:
                    results.append({"from": src, "to": dst, **txn})

        return results

    def to_networkx(
        self, asset: str | None = None, aggregation: str = "sum", include_metadata: bool = False
    ):
        ...
        try:
            ...
            raise ImportError(
                "NetworkX is required for graph export. " "Install it with: pip install networkx"
            )
        ...
        for src in edge_dict:
            for dst in edge_dict[src]:
                ...
                if txns:
                    edge_attrs["edge_type"] = txns[0].get(
                        "edge_type", int(classify_asset(txns[0]["asset"]))
                    )
    ...
    def summary(self) -> dict[str, Any]:
        ...
        total_transactions = sum(
            len(txns) for src_edges in self.edges.values() for txns in src_edges.values()
        )
        ...
        return {
            ...
            "edge_count": sum(len(dests) for dests in self.edges.values()),
