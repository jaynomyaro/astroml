...
class StreamService:
    ...
    async def _run_stream_with_monitoring(
        self, stream_id: str, stream: EnhancedStellarStream
    ) -> None:
        ...
        while self._running and retry_count < max_retries:
            try:
                ...
                delay = min(
                    stream.config.base_retry_delay * (2**retry_count), stream.config.max_retry_delay
                )
...
class MultiHorizonService:
    ...
    def add_horizon_service(self, horizon_url: str, stream_types: list[str]) -> None:
        ...
        for stream_type in stream_types:
            config = EnhancedStreamConfig(horizon_url=horizon_url, stream_type=stream_type)
...
async def run_single_stream(config: EnhancedStreamConfig) -> None:
    ...
    async with EnhancedStellarStream(config) as stream:
        ...
        logger.info(
            "Stream completed | processed=%d cursor=%s", stats["processed_count"], stats["cursor"]
        )
...
if __name__ == "__main__":
    ...
    config = EnhancedStreamConfig(
        horizon_url="https://horizon-testnet.stellar.org", stream_type="effects", cursor=None
    )
