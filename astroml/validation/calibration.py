...
class CalibrationAnalyzer:
    ...
    def compute_calibration_curve(
        self, y_true: np.ndarray, y_prob: np.ndarray, sample_weight: np.ndarray | None = None
        ...
        fraction_of_positives, mean_predicted_probability = calibration_curve(
            y_true, y_prob, n_bins=self.n_bins, strategy=self.strategy, sample_weight=sample_weight
        )
    ...
    def _compute_ece(
        ...
        if sample_weight is not None:
            ...
            bin_counts = [len(y_prob[self._get_bin_mask(y_prob, i)]) for i in range(len(mean_pred))]
    ...
    def plot_calibration_curve(
        ...
        ax1.fill_between(
            mean_pred, fraction_pos - stderr, fraction_pos + stderr, alpha=0.2, color="blue"
        )
        ...
        sm = plt.cm.ScalarMappable(
            cmap=plt.cm.YlOrRd, norm=plt.Normalize(vmin=0, vmax=max(bin_counts))
        )
        sm.set_array([])
    ...
    def plot_multiple_models(
        ...
        for i, (model_name, (y_true, y_prob)) in enumerate(models_data.items()):
            ax3.hist(y_prob, bins=20, alpha=0.5, label=model_name, color=colors[i], density=True)
...
def create_sample_fraud_data(
    n_samples: int = 10000, fraud_rate: float = 0.1
) -> tuple[np.ndarray, np.ndarray]:
    """
