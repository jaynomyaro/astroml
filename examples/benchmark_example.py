...
def run_basic_benchmark():
    ...
    config = create_config_from_template(
        ...
        data_params={"num_nodes": 500, "num_features": 16, "num_edges": 2000, "num_classes": 2},
        training_params={"epochs": 50, "batch_size": 32, "learning_rate": 0.01},
    )
...
def run_multiple_models():
    ...
    for model_name, task_type in models:
        ...
        config = create_config_from_template(
            ...
            data_params={"num_nodes": 300, "num_features": 16, "num_edges": 1000},
            training_params={"epochs": 30, "batch_size": 16},
        )
        ...
        try:
            ...
            if result.metrics:
                ...
                print(
                    f"  Key metrics: {', '.join(f'{k}: {v:.3f}' for k, v in result.metrics.items() if k in key_metrics)}"
                )
