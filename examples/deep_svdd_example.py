...
def create_synthetic_fraud_data(
    n_normal: int = 5000, n_fraud: int = 200, n_features: int = 12, random_state: int = 42
...
def demonstrate_advanced_training():
    ...
    for loss_type in loss_types:
        ...
        detector = FraudDetectionDeepSVDD(input_dim=X.shape[1], hidden_dims=[128, 64, 32], nu=0.07)
    ...
    for loss_type, metrics in results.items():
        print(
            f"{loss_type:15}: ROC={metrics['auc_roc']:.3f}, PR={metrics['auc_pr']:.3f}, F1={metrics['f1']:.3f}"
        )
...
def main():
    ...
    print(
        f"Best Loss Function: {max(advanced_results.keys(), key=lambda k: advanced_results[k]['auc_roc'])}"
    )
