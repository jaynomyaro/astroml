...
def test_check_isolated_nodes_raises_error():
    ...
    with pytest.raises(graph_validation.GraphValidationError):
        graph_validation.check_isolated_nodes(edges, all_nodes=all_nodes, allow_isolated=False)


def test_check_edge_consistency_self_loops():
    """Test detection of self-loops."""
    edges = pd.DataFrame({"source": ["A", "B", "C"], "target": ["A", "C", "D"]})
...
def test_check_edge_consistency_self_loops_not_allowed():
    ...
    edges = pd.DataFrame({"source": ["A", "B"], "target": ["A", "C"]})
...
def test_check_edge_consistency_duplicates():
    ...
    edges = pd.DataFrame({"source": ["A", "B", "A"], "target": ["B", "C", "B"]})
...
def test_check_edge_consistency_duplicates_not_allowed():
    ...
    edges = pd.DataFrame({"source": ["A", "A"], "target": ["B", "B"]})
...
def test_check_edge_consistency_null_values():
    ...
    edges = pd.DataFrame({"source": ["A", None, "C"], "target": ["B", "C", "D"]})
...
def test_check_edge_consistency_negative_weights():
    ...
    edges = pd.DataFrame(
        {"source": ["A", "B", "C"], "target": ["B", "C", "D"], "weight": [1.0, -2.0, 3.0]}
    )
...
def test_graph_summary_statistics_with_weights():
    ...
    edges = pd.DataFrame({"source": ["A", "B"], "target": ["B", "C"], "weight": [10.0, 20.0]})
...
def test_graph_summary_statistics_density():
    ...
    edges = pd.DataFrame({"source": ["A", "B"], "target": ["B", "A"]})
    stats = graph_validation.graph_summary_statistics(edges)
...
def test_validate_graph_comprehensive():
    ...
    report = graph_validation.validate_graph(edges, weight_col="weight", verbose=False)
...
def test_validate_graph_with_issues():
    ...
    edges = pd.DataFrame({"source": ["A", "A", "B"], "target": ["A", "A", "C"]})

    report = graph_validation.validate_graph(
        edges, allow_self_loops=True, allow_duplicates=True, verbose=False
    )
...
def test_custom_column_names():
    ...
    edges = pd.DataFrame({"from": ["A", "B"], "to": ["B", "C"], "amount": [100, 200]})

    stats = graph_validation.graph_summary_statistics(
        edges, source_col="from", target_col="to", weight_col="amount"
    )
...
def test_missing_columns_raises_error():
    ...
    edges = pd.DataFrame({"source": ["A", "B"], "wrong_col": ["B", "C"]})
